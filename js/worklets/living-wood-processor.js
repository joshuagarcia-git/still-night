/**
 * Living Wood AudioWorklet Processor
 *
 * Replaces 15 Tone.js nodes (7× Synth + 7× Gain + 1× branchBus Gain) with a
 * single AudioWorkletProcessor that runs 7 fatsine voices in one tight loop.
 *
 * Each voice: 3 detuned sine oscillators (fatsine spread:12 cents) with
 * per-voice gain. All amplitude control is external — the main thread computes
 * LFO breathing, bow crossfade, portato, velocity, and activation envelope,
 * then sends final per-note gain values via MessagePort each frame.
 *
 * Mono output: all 7 voices sum to a single channel, matching the existing
 * mono branchBus → cypressMixBus (channelCount=1) topology. Stereo imaging is
...
const SPREAD_CENTS = 12;   // ±12 cents detuning
const VOICE_VOLUME_DB = -6;
const VOICE_VOLUME_LINEAR = Math.pow(10, VOICE_VOLUME_DB / 20); // 0.5012
const BUS_GAIN = 0.5;      // branchBus attenuation
const BASE_AMPLITUDE = VOICE_VOLUME_LINEAR * BUS_GAIN; // ~0.2506

// Detuning multipliers: center, +spread, -spread
const DETUNE_UP = Math.pow(2, SPREAD_CENTS / 1200);    // 1.00694
const DETUNE_DOWN = Math.pow(2, -SPREAD_CENTS / 1200);  // 0.99311

// Envelope constants — same as WH (fast attack for pluck, long release for drone character)
const ATTACK_TIME = 0.05;   // 50ms
const RELEASE_TIME = 1.5;   // 1.5s
...
class LivingWoodProcessor extends AudioWorkletProcessor {
  constructor(options) {
    super();
    ...
    // Per-voice state
    this.phases = new Float64Array(VOICE_COUNT * OSCS_PER_VOICE); // 21 phase accumulators
    ...
    // Gain smoothing — 30ms time constant to match LW's heavier tau (vs WH's 5ms)
    // This preserves the organic, slow-settling character of the branch voices
    this.gainSmooth = 1.0 - Math.exp(-1.0 / (0.03 * sRate));
    ...
    for (let i = 0; i < VOICE_COUNT; i++) {
      const freq = notes[i] || 98;  // default G2
    ...
    this.port.onmessage = (e) => {
      ...
      switch (msg.type) {
        ...
        case 'frequencies':
          ...
          // phase-increment discontinuity is smoothed by the 30ms amplitude
          // envelope (LW's slower gainSmooth).
  ...
  process(inputs, outputs, parameters) {
    ...
    for (let s = 0; s < blockSize; s++) {
      ...
      for (let i = 0; i < VOICE_COUNT; i++) {
        ...
        // 3-oscillator fatsine: wavetable lookup
        const base = i * OSCS_PER_VOICE;
...
registerProcessor('living-wood-processor', LivingWoodProcessor);
